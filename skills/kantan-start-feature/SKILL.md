...
2. **Confirm the branch.** Work happens on the **current branch**. Do NOT create git worktrees. Do NOT create or switch branches unless the user explicitly asks. Tell the user which branch you are on.

3. **Locate the artifact root (backend).**
   - If a `.kantan-dev/` directory already exists in a workspace root, use that root.
   - Otherwise, use the root containing a `Gemfile` (Rails backend, e.g. has `config/application.rb`).
   All artifacts live under `<backend-root>/.kantan-dev/`.
