...
2. **Collect the stack facts once, yourself.** The three lenses must not each re-discover the repo — that is where the tokens go. Read only what you need: the backend `Gemfile`, the frontend `package.json` (the one depending on `react`), and each root's entry conventions file (`CLAUDE.md` in Claude Code, `AGENTS.md` in Codex and Cursor, or the other one if that is what exists). Skip the scoped topic files; the brief does not need them. From that, write one short brief: the languages and framework versions, the state and HTTP libraries, and any existing domain concept the subject touches. Keep the brief under 200 words.
